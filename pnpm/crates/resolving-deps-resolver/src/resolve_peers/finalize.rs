...
pub(super) struct WalkedNodePeers<'a> {
    /// Every peer resolved anywhere in this node's subtree.
    pub(super) resolved: &'a HashMap<String, NodeId>,
    pub(super) missing: &'a HashMap<String, MissingPeerInfo>,
    /// The subset of the above this node declares itself.
    pub(super) own_resolved: &'a HashMap<String, NodeId>,
    pub(super) is_pure: bool,
}
...
impl Walker<'_> {
    ...
    pub(super) fn record_walked_node(&mut self, mut node: WalkedNode<'_>) {
        ...
        let mut record_edges = self.previously_resolved_children(
            node.ancestry.parent_node_ids,
            node.ancestry.parent_pkg_ids_chain,
            &node.pkg.id,
        );
        ...
        let transitive_peer_dependencies =
            transitive_peer_names(node.pkg, node.peers.resolved, node.peers.missing);
        ...
        let record_order = self.output.next_record_order;
        self.output.next_record_order += 1;
        self.output.node_records.insert(
            node.node_id.clone(),
            NodeRecord {
                edges: record_edges,
        ...
        self.output
            ...
            .and_modify(|entry| {
                if entry.depth > node.ancestry.depth {
                    entry.depth = node.ancestry.depth;
                }
            })
    ...
    fn extend_record_edges(&mut self, edges: &mut BTreeMap<String, NodeId>, node: &WalkedNode<'_>) {
        let remapped_backedges =
            self.remapped_backedge_children(&node.pkg.id, node.children, node.ancestry.depth + 1);
        edges.extend(node.children.clone());
        for (alias, node_id) in remapped_backedges {
            edges.insert(alias, node_id);
        }
        for (peer_alias, peer_node_id) in node.peers.own_resolved {
