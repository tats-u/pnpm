...
impl WorkspaceTreeCtx {
    ...
    /// [`crate::resolve_dependency_tree::workspace_ctx::WorkspaceFinalizationState::finalization_pending`].
    pub(super) fn note_finalization_candidate(&self, pkg_id: &str) {
        if self.hooks.finalized_package.is_some() {
            lock_recoverable(&self.finalization.finalization_pending).push(Arc::from(pkg_id));
        }
    }
    ...
    /// [`WorkspaceTreeCtx::rebuild_discovery_tree`].
    pub(crate) fn sync_discovery_tree(
        ...
        let next = {
            let log = lock_recoverable(&self.tree.sync_log);
            SyncCursor {
    ...
    pub(crate) fn rebuild_discovery_tree(&self, tree: &mut ResolvedTree, cursor: &mut SyncCursor) {
        *cursor = {
        ...
        for (pkg_id, recorded) in lock_recoverable(&self.children.by_id).iter() {
            tree.children_by_id
                .entry(std::sync::Arc::<str>::clone(pkg_id))
                .or_insert_with(|| Arc::clone(&recorded.edges));
        }
        for (pkg_id, pkg) in lock_recoverable(&self.tree.packages).iter() {
            tree.packages
                .entry(std::sync::Arc::<str>::clone(pkg_id))
...
impl super::WorkspaceTreeStorage {
    ...
    /// [`WorkspaceTreeCtx::rebuild_discovery_tree`] instead of replaying the log.
