...
impl WorkspaceTreeCtx {
    ...
    pub fn snapshot(&self, direct: Vec<DirectDep>) -> ResolvedTree {
        ResolvedTree {
            direct,
            packages: lock_recoverable(&self.tree.packages).clone(),
            dependencies_tree: lock_recoverable(&self.tree.dependencies_tree).clone(),
            all_peer_dep_names: lock_recoverable(&self.tree.all_peer_dep_names).clone(),
            policy_violations: lock_recoverable(&self.policy.policy_violations).clone(),
            applied_patches: lock_recoverable(&self.policy.applied_patches).clone(),
    ...
    pub fn snapshot_reachable_from(&self, direct: Vec<DirectDep>) -> ResolvedTree {
        let dependencies_tree = lock_recoverable(&self.tree.dependencies_tree);
        ...
        let all_children = lock_recoverable(&self.children.by_id);
    ...
    pub(in super::super) fn run_preferred_versions(&self) -> MutexGuard<'_, RunVersionsCache> {
        ...
        let children_rewrites = self.tree.children_rewrites();
        let mut cache = lock_recoverable(&self.versions.run_versions_cache);
        ...
        let mut queue: Vec<String> = lock_recoverable(&self.versions.preferred_version_roots)
            .iter()
    ...
    pub(super) fn fold_settled_identities(&self, cache: &mut RunVersionsCache) {
        let identities = lock_recoverable(
            &self
                .versions
                .workspace_manifest_identities,
        );
    ...
    pub fn into_resolved_tree(mut self, direct: Vec<DirectDep>) -> ResolvedTree {
        let tree = ResolvedTree {
            direct,
            packages: take_locked(&mut self.tree.packages),
            dependencies_tree: take_locked(&mut self.tree.dependencies_tree),
            all_peer_dep_names: take_locked(&mut self.tree.all_peer_dep_names),
            policy_violations: take_locked(&mut self.policy.policy_violations),
            applied_patches: take_locked(&mut self.policy.applied_patches),
            children_by_id: take_locked(&mut self.children.by_id)
                .into_iter()
        ...
        // critical path.
        let dedup_caches = (
            take_locked(&mut self.cache.resolved_by_wanted),
            take_locked(&mut self.cache.resolved_workspace_by_wanted),
            take_locked(
                &mut self
                    .cache
                    .resolved_workspace_final_by_wanted,
            ),
            take_locked(&mut self.children.specs_by_id),
        );
...
impl super::WorkspaceChildrenState {
    ...
    pub(crate) fn record_first_walk_missing(
        ...
        let owners = lock_recoverable(&self.owner_by_id);
...
impl super::WorkspacePreferredVersions {
    /// Record importer-level direct-dependency package ids as roots for
    /// [`WorkspaceTreeCtx::run_preferred_versions`]. Called by [`fn@extend_tree`]
    ...
    /// [`WorkspaceTreeCtx::run_preferred_versions`] to fold once the package is
    /// reachable.
