...
fn importer_snapshot_follows_lazy_edges_for_the_package_closure() {
    ...
    assert!(
        snapshot
            .packages
            .contains_key("lazy-child@1.0.0"),
        "a package reachable only through a lazy edge must stay in the snapshot",
    );
    assert!(
        snapshot
            .children_by_id
            .contains_key("root@1.0.0")
    );
    assert!(
        !snapshot
            .packages
            .contains_key("foreign@1.0.0")
    );
    assert!(
        !snapshot
            .children_by_id
...
fn owner_missing_record_is_written_once_per_generation() {
    ...
    ctx.children
        .record_first_walk_missing("pkg-a", &miss(&one_peer));
    assert_eq!(
        ctx.children
    ...
    ctx.children
        .record_first_walk_missing(".", &miss(&two_peers));
    ...
    ctx.children
        .record_first_walk_missing(".", &miss(&none));
    let recorded = ctx.children.first_walk_missing_by_pkg();
    assert!(
        recorded
            .get("pkg@1.0.0")
...
fn owner_scope_snapshots_are_shared_until_a_write_changes_the_map() {
    ...
    ctx.children
        .record_first_walk_missing(".", &missing);
...
fn run_preferred_versions_cover_only_packages_reachable_from_recorded_roots() {
    ...
    workspace
        .versions
        .record_preferred_version_roots(std::iter::once("root@1.0.0"));
...
fn run_preferred_versions_grow_with_new_roots_and_rebuild_on_children_rewrites() {
    ...
    workspace
        .versions
        .record_preferred_version_roots(std::iter::once("late@3.0.0"));
    workspace.tree.bump_revision();
    assert_eq!(
        bucket_versions(
            &workspace
...
fn run_preferred_versions_fold_workspace_manifest_identities_once_reachable() {
    ...
    workspace
        .versions
        .record_workspace_manifest_identity("link:packages/opt", "opt", "1.0.0");
