...
async fn annotated_tag_prefers_dereferenced_commit() {
    ...
    let commit = resolve_ref(&stub, "repo", "v1.0.0", None)
        .await
        .unwrap();
...
async fn partial_commit_ambiguous_branch_raises() {
    ...
    let err = resolve_ref(&stub, "repo", "deadbeef", None)
        .await
...
async fn partial_commit_matches_single_ref() {
    ...
    let commit = resolve_ref(&stub, "repo", "deadbeef", None)
        .await
        .unwrap();
...
async fn ambiguous_partial_commit_mismatch_errors() {
    ...
    let err = resolve_ref(&stub, "repo", "deadbf12", None)
        .await
        .expect_err("ambig");
...
async fn semver_range_picks_max_satisfying() {
    ...
    let commit = resolve_ref(&stub, "repo", "HEAD", Some("~0.0.38"))
        .await
        .unwrap();
...
async fn semver_no_match_lists_available_versions() {
    ...
    let err = resolve_ref(&stub, "repo", "HEAD", Some("^100.0.0"))
        .await
        .expect_err("err");
    match err {
