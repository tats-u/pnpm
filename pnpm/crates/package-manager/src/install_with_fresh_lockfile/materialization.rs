    ...
    Box::pin(async move {
        ...
        FreshMaterialization {
            ...
            resources: MaterializationResources {
                ...
                deps_requiring_build_sink: owned
                    .resolution
...
impl<Reporter: self::Reporter + 'static> FreshMaterialization<'_, Reporter> {
    async fn run(
        ...
        match allow_build_policy {
            Some(policy) => {
                self.materialize(built_lockfile, policy)
                    .await
            }
            None => {
                self.finish_lockfile_only(built_lockfile)
                    .await
            }
        }
    }
    ...
    async fn materialize(
        ...
        .await;
        let on_disk = self
            .run_on_disk(&built_lockfile, &scope, &mut plan, &allow_build_policy)
            .await?;
        self.persist_result(built_lockfile, on_disk)
            .await
    }
