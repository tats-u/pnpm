...
fn walker_multi_importer_emits_per_importer_direct_deps() {
    ...
    assert!(
        result
            .graph
            .contains_key(&modules.join("a"))
    );
    assert!(
        result
            .graph
            .contains_key(&modules.join("b"))
    );
    assert_eq!(
...
fn walker_multi_importer_emits_per_importer_hierarchy() {
    ...
    assert!(
        result
            .hierarchy
            .contains_key(&lockfile_dir),
        "root importer hierarchy missing"
    );
    assert!(
        result
            .hierarchy
            .contains_key(&importer_root),
...
fn walker_hoist_workspace_packages_false_keeps_importer_deps() {
    ...
    assert!(
        result
            ...
            .contains_key(Lockfile::ROOT_IMPORTER_KEY)
    );
    assert!(
        result
            .direct_dependencies_by_importer_id
            .contains_key("packages/foo")
    );
    assert!(
        result
            .hierarchy
