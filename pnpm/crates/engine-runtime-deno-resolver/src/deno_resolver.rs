...
pub enum DenoResolverError {
    ...
    ReadAssets(#[error(source)] ReadDenoAssetsError),
}

/// Deno runtime resolver entry point.
///
/// Holds:
/// * the throttled HTTP client used for the GitHub Releases / SHA256
///   requests, and
/// * the npm resolver instance the version-selection step delegates
///   to. An `Arc<dyn Resolver>` keeps the dependency loose so the
///   install layer can share one resolver instance between the
///   default chain and the runtime resolvers.
