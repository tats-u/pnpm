...
async fn records_optional_subdeps_with_platform_fields() {
    ...
    let parent_key = "@pnpm.e2e/support-different-architectures@1.0.0"
        .parse()
        .unwrap();
    ...
    let only_linux = "@pnpm.e2e/only-linux-x64-glibc@1.0.0"
        .parse()
        .unwrap();
    let metadata = env
        .packages
        .get(&only_linux)
        .expect("platform subdep recorded in packages");
...
async fn resolves_package_manager_dependencies_graph() {
    ...
    let platform_key: PackageKey = "@pnpm/linuxstatic-x64@11.0.0"
        .parse()
        .unwrap();

    assert_eq!(env.packages[&pnpm_key].has_bin, Some(true));
    assert_eq!(
        env.packages[&pnpm_key]
            .engines
            .as_ref()
            .unwrap()["node"],
        ">=22.0.0"
    ...
    let detect_libc_ref = exe_snapshot
        .dependencies
        .as_ref()
        .unwrap()[&detect_libc_name]
        .to_string();
...
async fn resolves_package_manager_dependencies_with_exe_at_first_published_version() {
    ...
    let platform_key: PackageKey = "@pnpm/macos-arm64@6.17.1"
        .parse()
        .unwrap();
