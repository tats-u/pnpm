...
async fn an_upload_accumulates_across_appends() {
    ...
    let upload = storage
        .begin_blob_upload(&image("acme/app"))
        .await
        .unwrap();
...
async fn a_multi_component_repository_name_is_listed() {
    ...
    for name in ["acme/app", "acme/team/tool", "alpine"] {
        storage
            .write_hosted_document_if_current(&image(name), b"{}", None)
            .await
            .unwrap();
    }
...
async fn an_in_progress_upload_is_not_a_repository() {
    ...
    storage
        .write_hosted_document_if_current(&image("acme/app"), b"{}", None)
        .await
        .unwrap();
    storage
        .begin_blob_upload(&image("acme/app"))
...
async fn the_sweep_reclaims_only_uploads_that_have_gone_quiet() {
    ...
    assert_eq!(
        storage
            .sweep_blob_uploads(Duration::from_hours(1))
            .await
            .unwrap(),
        0,
    );
    assert!(
        storage
            .open_blob_upload(&image("acme/app"), stale.id())
            .await
            .unwrap()
            .is_some(),
    );
...
async fn an_upload_belongs_to_the_repository_that_started_it() {
    ...
    assert!(
        storage
            .open_blob_upload(&image("acme/other"), &id)
            .await
            .unwrap()
            .is_none(),
    );
...
async fn s3_sessions_are_repository_and_namespace_bound_and_expire() {
    ...
    assert_eq!(
        second
            .sweep_blob_uploads(Duration::from_hours(24))
            .await
            .unwrap(),
        0,
    );
