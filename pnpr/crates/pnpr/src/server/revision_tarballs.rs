...
pub(super) async fn serve_upstream_revision_tarball(
    ...
    let write = match state
        .inner
        .storage
        .open_upstream_revision_blob_tmp(&namespace, digest)
        .await
    {
        Ok(write) => write,
        Err(err) => return err.into_response(),
...
pub(super) async fn open_hosted_revision_tarball(
    ...
    match storage
        .open_hosted_blob(package, &filename)
        .await
    {
...
pub(super) async fn cached_revision_tarball(
    ...
    match state
        .inner
        .storage
        .open_upstream_revision_blob(namespace, digest)
        .await
    {
        Ok(Some((file, len))) => {
