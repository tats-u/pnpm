...
async fn private_hosted_org_masks_dist_tags_before_the_package_acl() {
    ...
    let resp = app
        .oneshot(
            Request::get("/-/package/@acme%2Fwidget/dist-tags")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn hosted_digest_route_rechecks_package_access() {
    ...
    config
        .routing
        .hosted
        .insert("corp".to_string(), hosted_with_access("corp", "alice"));
    config.routing.registries = Registries::new(
        vec![("corp".to_string(), Registry::Hosted { patterns: vec![] })]
            .into_iter()
            .collect(),
        Some("corp".to_string()),
    );
...
async fn hosted_registry_patterns_bound_publish_and_reads_on_every_path() {
    ...
    registries
        .validate()
        .expect("patterned hosted config is valid");
    ...
    let accepted = app
        .clone()
        .oneshot(publish_to("/~acme/@acme/widget", "@acme/widget"))
        .await;
...
async fn building_the_server_rejects_an_unsafe_programmatic_hosted_org() {
    ...
    config
        .routing
        .hosted
        .insert("evil".to_string(), hosted_with_access("../escape", "$all"));
...
async fn private_hosted_registry_denies_writes_from_non_members() {
    ...
    let retag_denied = app
        .clone()
        .oneshot(retag_as(&outsider))
        .await
        .unwrap();
    ...
    let retag_allowed = app
        .oneshot(retag_as(&member))
        .await
        .unwrap();
...
async fn search_does_not_enumerate_a_private_flat_root_registry() {
    ...
    for url in ["/-/v1/search?text=secret", "/-/v1/search?browse=true"] {
        ...
        config
            .routing
            .hosted
            .insert("corp".to_string(), hosted_with_access("", "alice"));
        config.routing.registries = Registries::new(
            vec![("corp".to_string(), Registry::Hosted { patterns: vec![] })]
                .into_iter()
                .collect(),
        ...
        for authorization in [None, Some(format!("Bearer {outsider}"))] {
            let response = app
                .clone()
                .oneshot(search_with(url, authorization))
                .await
                .unwrap();
            assert_eq!(response.status(), StatusCode::OK);
        ...
        let response = app
            .oneshot(search_with(url, Some(format!("Bearer {member}"))))
            .await
            .unwrap();
        assert_eq!(response.status(), StatusCode::OK);
        let body = body_json(response.into_body()).await;
