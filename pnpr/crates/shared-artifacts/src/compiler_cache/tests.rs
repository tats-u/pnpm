...
async fn local_entries_survive_restart_and_are_isolated_by_owner_and_key() {
    ...
    assert!(
        store
            .publish_compiler_cache("ci", &input, Bytes::from_static(b"compiled"))
            .await
            .unwrap(),
    ...
    assert_eq!(
        store
            .read_compiler_cache("other", &input)
            .await
            .unwrap(),
        None
    );
    assert_eq!(
        store
            .read_compiler_cache("ci", &key("different"))
            .await
            .unwrap(),
        None,
    );
    ...
    let results = join_all(
        replicas
            ...
            .map(|(index, store)| {
                ...
                async move {
                    store
                        .publish_compiler_cache("ci", input, Bytes::from(vec![index as u8]))
                        .await
                        .unwrap()
                }
            }),
...
async fn quota_rejects_new_entries_but_allows_hits_and_retries() {
    ...
    assert!(
        !store
            .publish_compiler_cache("ci", &input, Bytes::from_static(b"b"))
            .await
            .unwrap(),
        "a retry must work at the quota limit",
    );
    let rejected = store
        .publish_compiler_cache("ci", &key("two"), Bytes::from_static(b"b"))
        .await;
...
async fn corrupted_and_relocated_entries_are_never_served() {
    ...
    store
        .store
        .put(&relocated, bytes.into())
        .await
        .unwrap();
    let result = store
        .read_compiler_cache("ci", &key("relocated"))
        .await;
    assert!(result.is_err(), "served relocated entry: {result:?}");
    store
        .store
        .put(&original, Bytes::from_static(b"truncated").into())
        .await
        .unwrap();
    let result = store
        .read_compiler_cache("ci", &input)
...
async fn metadata_and_duplicate_publication_do_not_read_or_verify_payloads() {
    ...
    store
        .store
        .put(&path, vec![0; DIGEST_SIZE + 8].into())
        .await
        .unwrap();
    assert_eq!(
        store
    ...
    store
        .store
        .put(&path, Bytes::from_static(b"truncated").into())
        .await
        .unwrap();
    assert!(
        store
