...
impl SharedArtifactStore {
    ...
    pub(in super::super) async fn recover_expired_publication(
        ...
        *reclamation_needed = true;
            ...
            .await
        {
            ...
            self.store
                .delete(&self.object_path(&prepared.variant_path))
                .await?;
            return Err(error);
        }
    ...
    pub(in super::super) async fn recover_after_expiry(
        ...
        self.verify_stored_files(owner, variant_path, payload)
            .await?;
        ...
        let held = self
            .retake_scopes(owner, entry, holder, &scopes, &mut retaken)
            .await?
            && self
                .other_vocabulary_is_free(owner, entry, holder, payload)
                .await?;
        if held {
            return Ok(());
        }
        ...
        self.store
            .delete(&self.object_path(variant_path))
            .await?;
        self.release_retaken_scopes(owner, entry, holder, &retaken)
            .await?;
    ...
    pub(in super::super) async fn verify_stored_files(
        ...
        for file in &payload.manifest.added {
            ...
            let Some(bytes) = self
                .read_object_bounded(&path, file.size)
                .await?
            else {
                self.store
                    .delete(&self.object_path(variant_path))
    ...
    pub(in super::super) async fn retake_scopes(
        ...
        for scope in scopes {
            ...
            if self
                .create_object(&path, holder.to_string())
                .await?
            {
    ...
    pub(in super::super) async fn other_vocabulary_is_free(
        ...
        match compatibility_scopes(&payload.compatibility) {
            ...
            CompatibilityScopes::These(_) => Ok(self
